...
export default function EditDriftOffQuestionPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  ...
  useEffect(() => {
    const fetchQuestion = async () => {
      try {
        const result = await driftOffQuestionsApi.getQuestionById(id);
        if (result.success && result.data) {
          const question = result.data as DriftOffQuestion;
          setFormData({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const result = await driftOffQuestionsApi.updateQuestion(id, {
