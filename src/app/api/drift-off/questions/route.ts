...
export async function GET(req: NextRequest) {
  try {
    ...
    return NextResponse.json(successResponse('Drift Off questions fetched successfully', questions));
  } catch (error) {
...
export async function POST(req: NextRequest) {
  try {
    ...
    return NextResponse.json(successResponse('Drift Off question created successfully', question, 201), {
      status: 201,
    });
  } catch (error) {
    const { message, statusCode, error: errData } = handleError(error);
