...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json(successResponse('Drift Off responses retrieved', responses));
  } catch (error) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json(successResponse('Drift Off response created', response, 201), {
      status: 201,
    });
  } catch (error) {
    const { message, statusCode, error: errData } = handleError(error);
