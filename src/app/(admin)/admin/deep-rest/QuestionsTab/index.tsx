...
export default function QuestionsTab() {
  const [questions, setQuestions] = useState<DeepRestQuestion[]>([]);
  ...
  const fetchQuestions = useCallback(async () => {
    ...
    try {
      const result = await deepRestQuestionsApi.getQuestions(true);
      if (result.success && result.data) {
        setQuestions(result.data as DeepRestQuestion[]);
      }
    } catch {
    } finally {
  ...
  const confirmDelete = async () => {
    ...
    setDeleting(true);
    try {
      const result = await deepRestQuestionsApi.deleteQuestion(deleteConfirmation.id);
      if (result.success) {
        fetchQuestions();
      ...
      {questionsLoading ? (
        ...
        <>
          <div className={styles.topActions}>
            <Link href="/admin/deep-rest/questions/add" className={styles.addButton}>
          ...
          {questions.length === 0 ? (
            <div className={styles.emptyState}>
              ...
              <p>Create your first Deep Rest question to get started.</p>
              <Link href="/admin/deep-rest/questions/add" className={styles.addButton}>
                Add Question
              </Link>
            </div>
                  ...
                  {currentQuestion && (
                    ...
                    >
                      ...
                      <div className={styles.questionActions}>
                        <Link
                          href={`/admin/deep-rest/questions/edit/${currentQuestion.questionId}`}
