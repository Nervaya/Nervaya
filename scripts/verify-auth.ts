...
async function testAuth() {
    ...
    try {
        ...
        const loginCust = await axios.post(`${API_URL}/auth/login`, { email: customer.email, password: customer.password });
        console.log('   Success:', loginCust.data.success);
        const custToken = loginCust.data.data.token;

        // 4. Login Admin
        console.log('\n4. Logging in Admin...');
        const loginAdmin = await axios.post(`${API_URL}/auth/login`, { email: admin.email, password: admin.password });
